...
\begin{document}
	...
	\begin{enumerate}
		...
		\bfseries \item A \textit{k}-ad rendű korreláció közelítésének rendje és menetszáma \\
	...
	\par
	Ebben a megvalósításban a klasszikus, módosítások nélküli algoritmust valósítottam meg, és ezt vizsgáltam, hogy a generált sorozatok hogyan viszonyulnak a megvalósított mértékekhez.
	...
	Az additív karakteres konstrukció a Legendre szimbólumoshoz áll legközelebb. A Legendre szimbólumos konstrukció jobb véletlenségi tulajdonságokat mutat (lásd pl.: \cite{vikitoth1}, \cite{vikitoth2}). Az additív karakteres sorozat azonban nagyobb sorozatcsaládot képes előállítani, és implementációja egyszerűbb.
	...
	Az $a <<<$= $b$ az $a$ 32 bites változó balra forgatását jelöli $b$-vel. A $b$-vel való balra forgatás egy bitsorozat balra shiftelését (a magasabb helyiértékek felé shiftelünk) jelenti $b$-szer úgy, hogy a magas helyi értéken túlmenő bitek visszakerülnek a legkisebb helyi értékre (más néven: ciklikus shiftelés). A forgatás eredménye szintén az $a$ változóba kerül.
	\subsubsection*{Algoritmus}
	...
	\end{enumerate}
	Nyilvánvalóan a tetszőlegesen hosszú stream generáláshoz az $O$ bitsorozat nem elég. A fenti algoritmust $ChaChaRound()$-dal jelölve a stream generálás algoritmusa a következőféleképpen alakul:
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item Csökkentsük $l$-t 64-gyel, vagyis legyen $l := l - 64$.
		\end{enumerate}
	...
	Azt is megmutatta 2013-ban Nicky Mouha és Bart Preneel, hogy a Salsa 128-bites biztonságot jelent a differenciális kriptoanalízises támadásokkal szemben. \cite{salsadiffcrypt} Ezek olyan támadásokat jelentenek, melyek a bemenet megváltoztatásából a kimenet változásaira próbálnak következtetéseket levonni.
	\subsection{Mértékek}
	\subsubsection*{Ismertető}
	...
	Ekkor a \textbf{jól-eloszlási mértéket} definiáljuk a következőféleképpen:
	...
	Ekkor a \textbf{\textit{k}-ad rendű normalitás mértéket} a következőképpen definiáljuk:
	...
	\par
	Ekkor definiálhatjuk a \textbf{\textit{k}-ad rendű korreláció mértéket} a következőféleképpen:
	...
	\end{enumerate}
	A legfontosabb mértékek között vannak a fent tárgyalt jól-eloszlás-, $k$-ad rendű normalitás-, normalitás-, $k$-ad rendű korreláció- és korreláció-mértékek. Ezen kívül fontos még a \textbf{\textit{k}-ad rendű kombinált mérték}, ez a jól-eloszlást és $k$-ad rendű korrelációt ötvözi és az ehhez tartozó \textbf{kombinált mérték}, mely a normalitáséhoz hasonlóan adódik a $k$-ad rendű mértékből.
...
\begin{enumerate}
	\item Hozzuk létre a kódolást reprezentáló adatszerkezetet. Ajánlott valamilyen formájú hash táblát használni, hogy az elemek elérése konstans műveletigényű legyen. Jelen megvalósításban ez két std::unordered\_map-pal valósul meg, hogy mindkét irányból el tudjuk érni az adatokat.
...
Ez a fejezet a program implementációs döntéseibe kíván betekintést adni. A megvalósítás során tapasztalt nehézségekről és a program korlátairól is itt értekezem.
...
\begin{enumerate}
	...
	\normalfont A megvalósított pszeudovéletlen konstrukciók osztályait soroljuk ide.
...
\begin{enumerate}
	\item Megnézzük, hogy $a$ páros-e, ezt gyorsan meghatározhatjuk, hogy $a$-nak a bitenkénti és műveletét vizsgáljuk az $1$-et reprezentáló bitsorozattal. Ebből következik, hogyha $a$ \& $1 = 0$ (ahol az \& a bitenkénti és művelet), akkor $a$ páros, és ekkor a következő redukciót használjuk:
	\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Utólag ki kell számolni $(-1)^{\frac{p-1}{2}\frac{a-1}{2}}$-t. Ez nyilvánvalóan akkor lesz $-1$, ha $\frac{p-1}{2}\frac{a-1}{2} = \frac{(p-1)(a-1)}{4}$ páratlan. Mivel $a$ és $p$ is páratlanok, ezért mindenképpen igaz, hogy ez a számláló osztható 4-el, így megint elég csak a 4-gyel vett kvócienst megnézni, tehát ha $(p-1)(a-1)$ \& $4 \not=0$, akkor a fenti kifejezés $-1$, egyébként 1.
	\end{enumerate}
\end{enumerate}
...
Public metódus, inicializálja az adattagokat. Ezután még szükséges egy SetKey hívás, hogy megfelelően beállítsuk a kulcsot.
...
Az algoritmus negyedköreinek futtatásánál szükséges a ciklikus shiftelés megvalósítása a 32 bites szavakon. C++-ban erre nincs beépített eszköz, csak sima shiftelésre, de mivel tudjuk, hogy minden szó 32 bites, ezért egy trükkel megvalósítható a ciklikus változat is. Ha van egy $b$ 32 bites szavunk és $n$-nel szeretnénk ciklikusan balra shiftelni, akkor a következő kódrészletben vett kifejezésnek pontosan a $b$ $n$-nel vett ciklikus balra shiftelése az eredménye (C++ stílusú pszeudokódként): $(b << n) | (b >> (32-n));$. Mivel a túlcsorduló biteket elvesztjük és így a shiftelés irányától függően a megfelelő helyiértékeken 0-ák szerepelnek (ahova ciklikusan át kéne kerülnie a túlcsorduló elemeknek). Így ha bitenként vagyot alkalmazunk a két bitsorozatra, akkor a 0-ás helyiértékek helyén pontosan a túlcsordult részlet lesz.
...
A titkosítást végző eljárás. Az eredményt és a paramétert is QString-ben tároljuk, hogy a karakterek Unicode-dal legyenek továbbra is reprezentálva.
...
Ez a függvény a moduláris szorzást valósítja meg. \textit{a}$\cdot$\textit{b}-t számolja ki a \textit{mod} 
maradékosztályban.
...
Megkeresi az \textit{n} szám összes valódi osztóját és ezeket egy std::vector-ba helyezve adja vissza (minden osztó csak egyszer szerepel benne).
...
A jól-eloszlás mérték kiszámolása a legegyszerűbb a 3 közül, itt is fontos volt azonban, hogy a megvalósításnál a matematikai definíciót ne pazarlóan valósítsuk meg. A jól-eloszlás valamilyen $a$ kezdőpozíciótól számolja a $b$ eltolással vett indexeken lévő bitek összegét minden lehetséges $M$ végpozícióig (bővebben ld. a Mértékek fejezetet).

Itt igazából végpozícióval lehet úgy tenni, mintha nem létezne mint ciklusváltozó, a maximum keresését egybe lehet futtatni a bitek összegzésével. Tehát csak a lehetséges kezdőpozíciók és eltolások szerint keresünk maximumot, és az összegek között számolás közben frissítjük a maximális összeget. 
...
\begin{enumerate}
	...
	\normalfont $m$ maradékosztályban \\ $f \in \mathbb{Z}_m[x], x \in \mathbb{Z}_m$ helyen. Vizsgált esetek és várt eredmények:
	\begin{enumerate}
	...
	\normalfont Legendre-konstrukció szerinti generálás, 10 hosszú sorozat, $p = 101$, $f \in \mathbb{Z}_p[x], f(x) = (x-1)(x-5)$, várt eredmény:	$1101011010$.
	...
	\normalfont Additív karakteres generálás, 10 hosszú sorozat, $p = 23$, $f \in \mathbb{Z}_p[x], f(x) = (x-1)(x-5)$. Várt eredmény: 1100011001
...
Grafikonok megjelenítése és statisztikák számolásának implementálása a számolt mértékekből is egy célszerű kiegészítési lehetőség. Ezekkel tovább lehet elemezni a megvalósított konstrukciókat.
...
A programmal tett vizsgálataim során azt tapasztaltam, hogy a Legendre és additív karakteres konstrukció az eddig belátott tulajdonságok szerint (lásd:  \cite{largefam} és \cite{additive}) jó eredményeket ad a megvalósított mértékekkel.
...
A ChaCha20 konstrukció jól teljesített a mértékekkel, átlagosan jó eredmények születtek. Ez alátámasztja azt, hogy az eddigi vizsgálatok alapján (lásd: \cite{salsadiffcrypt} és \cite{salsaattack}) a Salsa/ChaCha család elég jó tulajdonságokkal rendelkező generátorcsoport.
...
\begin{thebibliography}{3}
	...
	\bibitem{vikitoth1}
	V. Tóth: \textit{The study of collision and avalanche effect in a family of pseudorandom binary sequences}, Periodica Math. Hungar., 59, 2009, [1-8].
	\bibitem{vikitoth2}
	V. Tóth: \textit{Ütközés és lavinahatás vizsgálata véges pszeudovéletlen sorozatokban}, Doktori értékezés, ELTE, Informatika Doktori Iskola, 2014
	\bibitem{rcleak}
